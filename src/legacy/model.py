...
images = np.array(['../data/iam/words/' + x['id'] + '.png' for x in data])
...
def build_LSTM_model():
    ...
    mdlstm1 = LSTMCell4D(inp, 2, 4, return_sequences=True)
    mdlstm2 = LSTMCell4D(mdlstm1, 10, 16, return_sequences=True)
    mdlstm3 = LSTMCell4D(mdlstm2, 50, 40, return_sequences=True)
...
save_model((str(prediction_model.name)+str(batch_size)+'batchsize'+str(epochs)+'epochs'+'PREDICTION'), prediction_model)
